...
const mapRoamToApi = (v: string | undefined): string => {
  if (!v) return '';
  const s = v.trim().toLowerCase();
  if (s === 'si' || s === 's') return 'Si';
  if (s === 'no' || s === 'n') return 'No';
  if (s === 'posible' || s === 'posi' || s === 'p' || s === 'puede ser' || s === 'puede') return 'Posi';
  // Enviar literal si es otra etiqueta
  return String(v).trim();
};

const normalizeApiRoamToSelect = (apiValue: any): string => {
  if (apiValue == null || apiValue === '') return '';
  const s = String(apiValue).trim().toLowerCase();
  if (s === 'si' || s === 's') return 'Si';
  if (s === 'no' || s === 'n') return 'No';
  if (s === 'posi' || s === 'posible' || s === 'p') return 'Posible';
  // Si la API devuelve la etiqueta literal ya en español, intentar usarla tal cual
  const cap = String(apiValue).trim();
  return cap;
};

// estTrabColor: enviamos/recibimos el literal seleccionado directamente
...
const normalizeApiSiNoIgnora = (apiValue: any): 'Si' | 'No' | 'Ignora' | '' => {
  if (apiValue == null || apiValue === '') return '';
  if (typeof apiValue === 'boolean') return apiValue ? 'Si' : 'No';
  const s = String(apiValue).trim().toLowerCase();
  if (s === 's' || s === 'si' || s === 'true' || s === '1') return 'Si';
...
const buildBaseDenunciaPayload = (formData: DenunciaFormData, empresa?: any, empleadorCuit?: number) => {
  ...
  const prestadorTrasladoDigits = onlyDigits((formData as any).estTrabPrestadorTraslado || '');
  ...
  return {
    ...
    empOcCuit: Number(onlyDigits((formData as any).ocurrenciaCuit ?? '')),
    empOcRazonSocial: String((formData as any).ocurrenciaRazonSocial ?? ''),
    empOcEstablecimiento: String((formData as any).ocurrenciaRazonSocial ?? ''),
    empOcCiiu: Number(onlyDigits((formData as any).ocurrenciaCiiu ?? '')),
    empOcDomicilioCalle: String((formData as any).ocurrenciaCalle ?? ''),
    empOcDomicilioNro: String((formData as any).ocurrenciaNumero ?? ''),
    empOcDomicilioPiso: String((formData as any).ocurrenciaPiso ?? ''),
    empOcDomicilioDpto: String((formData as any).ocurrenciaDpto ?? ''),
    empOcDomicilioEntreCalle1: String((formData as any).ocurrenciaEntreCalle1 ?? ''),
    empOcDomicilioEntreCalle2: String((formData as any).ocurrenciaEntreCalle2 ?? ''),
    empOcCodLocalidad: String((formData as any).ocurrenciaCodLocalidad ?? ''),
    empOcCodPostal: Number(onlyDigits((formData as any).ocurrenciaCodPostal ?? '')),
    empOcSubContrato: "",
    empOcTelefonos: String((formData as any).ocurrenciaTelefonos ?? ''),
    empOceMail: String((formData as any).ocurrenciaEmail ?? ''),
   // Datos del Establecimiento (empEst*)
    ...
    empEstTelefonos: String(formData.establecimientoTelefono ?? (formData as any).establecimientoTelefonos ?? ''),
    ...
    estTrabEstaConsciente: mapSiNoToApi(formData.estTrabEstaConsciente || ''),
    estTrabColor: String(formData.estTrabColor || ''),
    estTrabHabla: mapSiNoToApi(formData.estTrabHabla || ''),
    estTrabGravedad: String(formData.estTrabGravedad || ''),
    estTrabRespira: mapSiNoToApi(formData.estTrabRespira || ''),
    estTrabObservaciones: String(formData.estTrabObservaciones ?? ''),
    estTrabTieneHemorragia: mapSiNoToApi(formData.estTrabTieneHemorragia || ''),
    estTrabContextoDenuncia: mapSiNoToApi(formData.estTrabContextoDenuncia || ''),
    trasladoTipo: String(formData.trasladoTipo ?? ''),
    estTrabPrestadorTraslado: prestadorTrasladoDigits ? Number(prestadorTrasladoDigits) : null,
    // ROAM
    roam: mapRoamToApi((formData as any).roam || ''),
    roamInterno: Number((formData as any).roamInterno ?? 0),
    roamNumero: Number((formData as any).roamNumero ?? 0),
    roamAnio: Number((formData as any).roamAnio ?? 0),
...
const transformFormDataToPostRequest = async (
  ...
  includeNroPreDenuncia: boolean = false
  ...
  return {
    ...base,
    ...(includeNroPreDenuncia ? { nroPreDenuncia: 0 } : {}),
    siniestroNro: 0,
    ...
    if (requestState.method === 'edit') {
      ...
      if (denunciaByIdData) {
        ...
        const mapped: DenunciaFormData = {
          ...
          empCuit: api.empCuit != null ? String(api.empCuit) : initialDenunciaFormData.empCuit,
          empPoliza: api.empPoliza != null ? String(api.empPoliza) : initialDenunciaFormData.empPoliza,
          empRazonSocial: api.empRazonSocial ?? initialDenunciaFormData.empRazonSocial,
          empDomicilioCalle: api.empDomicilioCalle ?? initialDenunciaFormData.empDomicilioCalle,
          empDomicilioNro: api.empDomicilioNro ?? initialDenunciaFormData.empDomicilioNro,
          empDomicilioPiso: api.empDomicilioPiso ?? initialDenunciaFormData.empDomicilioPiso,
          empDomicilioDpto: api.empDomicilioDpto ?? initialDenunciaFormData.empDomicilioDpto,
          empDomicilioEntreCalle1: api.empDomicilioEntreCalle1 ?? initialDenunciaFormData.empDomicilioEntreCalle1,
          empDomicilioEntreCalle2: api.empDomicilioEntreCalle2 ?? initialDenunciaFormData.empDomicilioEntreCalle2,
          empCodLocalidad: api.empCodLocalidad != null ? String(api.empCodLocalidad) : initialDenunciaFormData.empCodLocalidad,
          empCodPostal: api.empCodPostal != null ? String(api.empCodPostal) : initialDenunciaFormData.empCodPostal,
          empTelefonos: api.empTelefonos ?? initialDenunciaFormData.empTelefonos,
          empEmail: api.empeMail ?? initialDenunciaFormData.empEmail,
          ...
          roam: normalizeApiRoamToSelect(sin?.roam ?? api.roam ?? ''),
          roamNumero: String(sin?.roamNumero ?? api.roamNumero ?? initialDenunciaFormData.roamNumero),
          roamInterno: String(sin?.roamInterno ?? api.roamInterno ?? initialDenunciaFormData.roamInterno),
          roamAnio: String(sin?.roamAnio ?? api.roamAnio ?? initialDenunciaFormData.roamAnio),
          ...
          // Datos de Ocurrencia (solapa "Denuncia-Enfermedad Profesional")
          ocurrenciaCuit: api.empOcCuit != null ? String(api.empOcCuit) : initialDenunciaFormData.ocurrenciaCuit,
          ocurrenciaRazonSocial: api.empOcEstablecimiento ?? api.empOcRazonSocial ?? initialDenunciaFormData.ocurrenciaRazonSocial,
          ocurrenciaCiiu: api.empOcCiiu != null ? String(api.empOcCiiu) : initialDenunciaFormData.ocurrenciaCiiu,
          ocurrenciaCalle: api.empOcDomicilioCalle ?? initialDenunciaFormData.ocurrenciaCalle,
          ocurrenciaNumero: api.empOcDomicilioNro ?? initialDenunciaFormData.ocurrenciaNumero,
          ocurrenciaPiso: api.empOcDomicilioPiso ?? initialDenunciaFormData.ocurrenciaPiso,
          ocurrenciaDpto: api.empOcDomicilioDpto ?? initialDenunciaFormData.ocurrenciaDpto,
          ocurrenciaEntreCalle1: api.empOcDomicilioEntreCalle1 ?? initialDenunciaFormData.ocurrenciaEntreCalle1,
          ocurrenciaEntreCalle2: api.empOcDomicilioEntreCalle2 ?? initialDenunciaFormData.ocurrenciaEntreCalle2,
          ocurrenciaCodLocalidad: api.empOcCodLocalidad != null ? String(api.empOcCodLocalidad) : initialDenunciaFormData.ocurrenciaCodLocalidad,
          ocurrenciaCodPostal: api.empOcCodPostal != null ? String(api.empOcCodPostal) : initialDenunciaFormData.ocurrenciaCodPostal,
          ocurrenciaTelefonos: api.empOcTelefonos ?? initialDenunciaFormData.ocurrenciaTelefonos,
          ocurrenciaEmail: api.empOceMail ?? initialDenunciaFormData.ocurrenciaEmail,
          ...
          estTrabEstaConsciente: normalizeApiSiNoIgnora(api.estTrabEstaConsciente) || initialDenunciaFormData.estTrabEstaConsciente,
          estTrabColor: api.estTrabColor != null ? String(api.estTrabColor) : initialDenunciaFormData.estTrabColor,
          estTrabHabla: normalizeApiSiNoIgnora(api.estTrabHabla) || initialDenunciaFormData.estTrabHabla,
          estTrabGravedad: api.estTrabGravedad != null ? String(api.estTrabGravedad) : initialDenunciaFormData.estTrabGravedad,
          estTrabRespira: normalizeApiSiNoIgnora(api.estTrabRespira) || initialDenunciaFormData.estTrabRespira,
          estTrabObservaciones: String(api.estTrabObservaciones ?? initialDenunciaFormData.estTrabObservaciones),
          estTrabTieneHemorragia: normalizeApiSiNoIgnora(api.estTrabTieneHemorragia) || initialDenunciaFormData.estTrabTieneHemorragia,
          estTrabContextoDenuncia: normalizeApiSiNoIgnora(api.estTrabContextoDenuncia) || initialDenunciaFormData.estTrabContextoDenuncia,
          trasladoTipo: api.trasladoTipo ?? initialDenunciaFormData.trasladoTipo,
          estTrabPrestadorTraslado: api.estTrabPrestadorTraslado != null ? String(api.estTrabPrestadorTraslado) : initialDenunciaFormData.estTrabPrestadorTraslado,
    ...
    try {
      ...
      if (method === "create") {
        ...
        showModalMessage(
          isFinal
            ? (canRealizaDenuncias
                ? "Su denuncia ya a sido Registrada y se encuentra en Estado Pendiente"
  ...
  const baseTableColumns = [
    {
      ...
      cell: (info: any) => {
        ...
        const estado = (original?.estado ?? '').toString();
        if (estado === 'Borrador') return 0;
        const pre = original?.nroPreDenuncia ?? 0;
        if (pre && Number(pre) !== 0) return 0;
        return info.getValue ? info.getValue() ?? 0 : (original?.denunciaNro ?? 0);
      }
    },
    {
        ...
        title={
          modalMessage.type === 'error'
            ...
            : modalMessage.type === 'success'
              ? (canRealizaDenuncias ? 'Denuncia Realizada con Éxito' : 'Pre-Denuncia Registrada con Éxito')
              : undefined
        }
      />
    </Box>
